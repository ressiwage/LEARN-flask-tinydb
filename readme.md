...
# исправления, которые я бы внес
- сделать метод гетом вместо поста, т.к. используются url параметры, но не используется тело
- сделать сохранение даты в iso файле, т.к. это значительно облегчит дело если подключатся другие фреймворки и языки

# исправления, которые пришлось внести
- в url параметры нельзя передать "+" и " ", так что телефон обрабатывается как 7XXXXXXXXXX

# запуск
1. установите докер
2. запустите файл run.sh (для linux)
